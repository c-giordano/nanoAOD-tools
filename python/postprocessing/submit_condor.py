...
def runner_writer(sample, n, files, folder, training):
    ...
    f.write("python tree_skimmer_Tprime_vers2.py " + sample.label + " " + str(n) + " " + str(files) + " " + str(folder)+" "+str(training)+"\n")
...
for sample in samples:
    ...
    if(isMC):
        ...
        for i in range(len(files_list)/split+1):
            ...
            print("python tree_skimmer.py " + sample.label + " " + str(i) + " " + ",".join( e for e in files_list[split*i:split*(i+1)]) + " " + str(final_folder)+" "+str(training))
