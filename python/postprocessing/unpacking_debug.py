...
#        self.out.branch("GoodEl_mindR", "F", lenVar="nGoodEl")
    ...
    def analyze(self, event):
        ...
        if not ((len(goodMu)>0 or len(goodEl)>0) and len(goodJet)>0 ):
            ...
            if(verbose):print("nTop is ", nTop, " crosscheck ", nGoodMu," ", nGoodEl, "  ", nGoodJet)
                ...
                if m in goodMu:
                    ...
                    for l in LHE:
                        if((abs(l.pdgId) == 13)):
                            ...
                            print("n matching muons ",self.countmatchingmuons)
                            muon_LHE_match = True
                ...
                if j in goodJet:
                    ...
                    for l in LHE:
                        ...
                        if((abs(l.pdgId) == 13)) and deltaR(j.p4().Eta(),j.p4().Phi(),l.p4().Eta(),l.p4().Phi()) <0.4:
                            ...
                            goodmu_mindR_temp = closest(l, goodMu)
                            print("goodMu mindR in loop:", goodmu_mindR_temp)
                            if goodmu_mindR>0.1:
                        ...
                        if((abs(l.pdgId) == 11)) and deltaR(j.p4().Eta(),j.p4().Phi(),l.p4().Eta(),l.p4().Phi()) <0.4:
                            ...
                            goodel_mindR_temp = closest(l, goodEl)
                            if goodel_mindR>0.3:
                                is_jet_standalone = True

                    goodmu_mindR.append(goodmu_mindR_temp)
                    print("goodMu mindR outside the loop:", goodmu_mindR_temp)
                    ...
                    if(verbose):print("jet pt  ",j.p4().Pt()," is good?" , is_jet_true,  " cross check ",j.partonFlavour)

                    for m in allMu:
                        
                        if m in goodMu:
                            if(verbose):print("in good mu, jet pt", j.p4().Pt(), " muon pt is ",m.p4().Pt())
                            is_muon_prompt = is_mu_prompt_index[goodMu.index(m)]
                            if(verbose):print("is muon prompt? ", is_muon_prompt)
...
#                            good_muon_phi.append(good_muon_momentum.Phi())
                            ...
                            print("is truth 0?", (is_jet_true == True and is_muon_prompt== True and (j.partonFlavour*m.charge)>0))
                            print("is jet true",is_jet_true, " muon promppt ", is_muon_prompt, " partonFlavour ",j.partonFlavour, " muonvcharge ", m.charge )
                            ...
                            if((is_jet_true == True and is_muon_prompt== True and (j.partonFlavour*m.charge)>0)):
                                ...
                                print(" partial count true tops ", self.counttruetops)
...
#                               top_lhe_truth_val_el = (6)
        ...
        print(" final count true tops ", self.counttruetops)
