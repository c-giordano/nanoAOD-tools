...
float LHAPDFUncertaintiesCalculator::getUncertainty(float x1, float x2, int id1, int id2,float scalePDF, string newPDF, bool verbose){
  ...
  if (newPDF==PDF_){
    ...
    for (size_t imem = 0; imem < npdfmem_1; imem++) {
	...
	cout << " checking replica: weight "<<weights_pdfs[imem]<<endl;
	  cout<<"val is "<<" sum "<<mean<<" tempmean "<<mean/(imem+1) <<endl;
      }
    }
    mean=mean/npdfmem_1;
    for (size_t imem = 0; imem < npdfmem_1; imem++) {
      devst= (weights_pdfs[imem]-mean)*(weights_pdfs[imem]-mean);
      if(verbose)cout<<"val is "<<" sum dev "<<devst<<" tempdevst "<< devst/(imem+1)<<" tempsigma "<<sqrt(devst/(imem+1)) <<endl;
    }    
    if(verbose )cout << " sum dev tot "<< devst<<" devst"<<devst/npdfmem_1<< " sigma "<< sqrt(devst/(npdfmem_1-1))<<endl;
    sigma=sqrt(devst/(npdfmem_1-1));
  }
